...
namespace LazyPanClean {
    ...
    public class LPGenerate : EditorWindow {
        ...
        public static void GenerateConfig() {
            ...
            LPGameSetting lpGameSetting = LPLoader.LoadGameSetting();
            LPReadCSV.Instance.Read(obj.name, out string content, out string[] lines);
            GenerateConfigScript(obj.name, lpGameSetting, lines);
        }

        [MenuItem("Assets/Create/LazyPan/生成游戏框架根据流程配置")]
        public static void GenerateFrameByFlowConfig() {
            EditorWindow window = GetWindow(typeof(LPGenerate), true, "快速生成游戏框架根据流程配置");
            window.position = new Rect(new Vector2(Screen.width, Screen.height), new Vector2(500, 300));
            window.Show();
            window.Focus();
        }
        
        private static void GenerateConfigScript(string className, LPGameSetting lpGameSetting, string[] lines) {
            string property = "";
            string readContent = "";
            ...
            string inputPath = string.Concat(Application.dataPath, lpGameSetting.TxtPath, "GenerateConfigTemplate.txt");
            string outputPath = string.Concat(Application.dataPath, lpGameSetting.ConfigScriptPath);
                ...
                if (i > 2) {//遍历第三行到最后一行
                    ...
                    if (i + 1 == lines.Length || (i + 1 != lines.Length && lines[i + 1].Split(',')[0] != curFlowSign)) {
                        ...
                        foreach (string tmpui in openui) {
                            openuireplace += $"\t\t\t{tmpui} = LPUI.Instance.Open(\"{tmpui}\");\n";
                            /*界面属性*/
                            uifieldreplace += $"\t\tprivate LPComp {tmpui};\n";
                            getuireplace += $"\t\t/*获取UI*/\n\t\tpublic LPComp GetUI() {{\n\t\t\treturn {tmpui};\n\t\t}}\n";
                        }
                        ...
                        foreach (string tmpui in closeui) {
                            closeuireplace += $"\t\t\tLPUI.Instance.Close(\"{tmpui}\");\n";
                        }
                        ...
                        foreach (KeyValuePair<string, List<string>> keyValue in stage) {
                            ...
                            if (key == "Init") {
                                foreach (string tmpInit in keyValue.Value) {
                                    ...
                                    initloadentityreplace += $"\t\t\t{valStrs[1]} = LPObj.Instance.LoadEntity(\"{valStrs[1]}\");\n";
                                    /*实体属性*/
                                    entityfieldreplace += $"\t\tprivate LPEntity {valStrs[1]};\n";
                                }
                            } else if (key == "Clear") {
                                foreach (string tmpInit in keyValue.Value) {
                                    string[] valStrs = tmpInit.Split("|");
                                    unloadentityreplace += $"\t\t\tLPObj.Instance.UnLoadEntity({valStrs[1]});\n";
                                }
                            } else {
                                /*创建阶段方法开头*/
                                ...
                                foreach (string tmpInit in keyValue.Value) {
                                    ...
                                    if (valStrs[0] == "load_entity") {
                                        stagereplace += $"\t\t\t{valStrs[1]} = LPObj.Instance.LoadEntity(\"{valStrs[1]}\");\n";
                                        /*实体属性*/
                                        entityfieldreplace += $"\t\tprivate LPEntity {valStrs[1]};\n";
                                    } else if (valStrs[0] == "unload_entity") {
                                        stagereplace += $"\t\t\tLPObj.Instance.UnLoadEntity({valStrs[1]});\n";
                                    }
                                }
        ...
        public static void GenerateBehaviour(bool isTemplate) {
            /*读取配置*/
            LPReadCSV.Instance.Read("BehaviourGenerate", out string content, out string[] lines);
            LPLogUtil.Log(content);
